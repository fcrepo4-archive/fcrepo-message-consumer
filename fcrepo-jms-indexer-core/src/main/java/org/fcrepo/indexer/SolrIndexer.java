...
public class SolrIndexer implements Indexer {
	 ...
	 */
	@Autowired
	public SolrIndexer(final SolrServer solrServer, String customIndexerFieldsConfigFile) throws IOException, URISyntaxException {
		this.server = solrServer;
		this.customIndexerFieldsConfig=customFieldsConfigReader(customIndexerFieldsConfigFile);
	}
	...
	List<String> customFieldsConfigReader(String filepath)throws IOException, URISyntaxException{
		final List<String> result = new ArrayList<String>();
		final String FIELD_NAME_PATTERN = "name=";
		URL fileUrl = SolrIndexer.class.getResource(filepath);
		String config = new String(java.nio.file.Files.readAllBytes(Paths.get(fileUrl.toURI())), "UTF8");
		String[] lines = config.split(System.getProperty("line.separator"));
		for (int i = 0; i < lines.length; i++){
			final String thisline = lines[i];
			final int fieldnameFound = thisline.indexOf(FIELD_NAME_PATTERN);
			if (fieldnameFound != -1) {
				final int start = fieldnameFound + FIELD_NAME_PATTERN.length()
						+ 1;
				final int end = thisline.indexOf('"', start);
				result.add(thisline.substring(start, end));
			}
		}
